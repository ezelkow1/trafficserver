...
Acl::loadanonymous(const YAML::Node &anonNode)
{
  ...
  try {
    ...
    if (anonNode["vpn_datacenter"].as<bool>(false)) {
      TSDebug(PLUGIN_NAME, "saw vpn datacenter true");
      _vpn_datacenter = true;
    }

    if (anonNode["relay_proxy"].as<bool>(false)) {
      TSDebug(PLUGIN_NAME, "saw relay proxy true");
      _relay_proxy = true;
    }

    if (anonNode["proxy_over_vpn"].as<bool>(false)) {
      TSDebug(PLUGIN_NAME, "saw proxy over vpn true");
      _proxy_over_vpn = true;
    }

    if (anonNode["smart_dns_proxy"].as<bool>(false)) {
      TSDebug(PLUGIN_NAME, "saw smart dns proxy true");
...
Acl::eval_anonymous(MMDB_entry_s *entry)
{
  ...
  if (_vpn_datacenter) {
    ...
    if ((MMDB_SUCCESS == status) && (entry_data.has_data)) {
      if (entry_data.type == MMDB_DATA_TYPE_BOOLEAN) {
        if (entry_data.boolean == true) {
          TSDebug(PLUGIN_NAME, "saw is_vpn_datacenter set to true bool");
          return false;
        }
      }
    }
  ...
  if (_relay_proxy) {
    ...
    if ((MMDB_SUCCESS == status) && (entry_data.has_data)) {
      if (entry_data.type == MMDB_DATA_TYPE_BOOLEAN) {
        if (entry_data.boolean == true) {
          TSDebug(PLUGIN_NAME, "saw is_relay_proxy set to true bool");
          return false;
        }
      }
    }
  ...
  if (_proxy_over_vpn) {
    ...
    if ((MMDB_SUCCESS == status) && (entry_data.has_data)) {
      if (entry_data.type == MMDB_DATA_TYPE_BOOLEAN) {
        if (entry_data.boolean == true) {
          TSDebug(PLUGIN_NAME, "saw is_proxy_over_vpn set to true bool");
          return false;
        }
      }
    }
  ...
  if (_smart_dns_proxy) {
    ...
    if ((MMDB_SUCCESS == status) && (entry_data.has_data)) {
      if (entry_data.type == MMDB_DATA_TYPE_BOOLEAN) {
        if (entry_data.boolean == true) {
          TSDebug(PLUGIN_NAME, "saw is_smart_dns_proxy set to true bool");
          return false;
        }
      }
    }
